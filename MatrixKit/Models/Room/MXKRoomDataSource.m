...
- (void)limitMemoryUsage:(NSInteger)maxBubbleNb
{
    ...
    if (bubbleCount > maxBubbleNb)
    {
        ...
        [_room outgoingMessagesWithCompletion:^(NSArray<MXEvent *> * _Nullable outgoingMessages) {
    ...
    if (_isLive)
    {
        ...
        liveEventsListener = [_timeline listenToEventsOfTypes:liveEventTypesFilterForMessages onEvent:^(MXEvent *event, MXTimelineDirection direction, MXRoomState *roomState) {
            ...
            if (MXTimelineDirectionForwards == direction)
            {
                ...
                [self.room outgoingMessagesWithCompletion:^(NSArray<MXEvent *> * _Nullable outgoingMessages) {
                    __block MXEvent *localEcho;
                    
                    dispatch_group_t dispatchGroup = dispatch_group_create();
                    
                    if (outgoingMessages.count && [event.sender isEqualToString:self.mxSession.myUser.userId])
                    {
                        dispatch_group_enter(dispatchGroup);
                        [self.room pendingLocalEchoRelatedToEvent:event completion:^(MXEvent * _Nullable echoEvent) {
                            localEcho = echoEvent;
                            if (localEcho)
                            {
...
- (void)refreshSecondaryEventListeners:(NSArray *)liveEventTypesFilterForMessages
{
    ...
    if (_isLive)
    {
        ...
        secondaryLiveEventsListener = [_secondaryTimeline listenToEventsOfTypes:liveEventTypesFilterForMessages onEvent:^(MXEvent *event, MXTimelineDirection direction, MXRoomState *roomState) {
            ...
            {
                // Check for local echo suppression
                [self.secondaryRoom outgoingMessagesWithCompletion:^(NSArray<MXEvent *> * _Nullable outgoingMessages) {
                    __block MXEvent *localEcho;
                    dispatch_group_t dispatchGroup = dispatch_group_create();
                    
                    if (outgoingMessages.count && [event.sender isEqualToString:self.mxSession.myUserId])
                    {
                        dispatch_group_enter(dispatchGroup);
                        [self.secondaryRoom pendingLocalEchoRelatedToEvent:event completion:^(MXEvent * _Nullable echoEvent) {
                            localEcho = echoEvent;
                            if (localEcho)
                            {
                                // Check whether the local echo has a timestamp (in this case, it is replaced with the actual event).
